  ...
  (when (bound-and-true-p vc-mode)
    (let ((backend (vc-backend buffer-file-name))
      ...
      (concat
       (propertize " ")
       (case backend
         ('Git (all-the-icons-faicon "git" :v-adjust -0.05))
         ('SVN (all-the-icons-faicon "cloud" :v-adjust -0.05))
         ('Hg  (all-the-icons-faicon "cloud" :v-adjust -0.05))
         (t    (format "%s" vc-mode)))
       (propertize "_")
       (cond ((memq state '(edited added))
              (all-the-icons-octicon "git-branch" :v-adjust -0.05))
             ((eq state 'needs-merge)
              (all-the-icons-octicon "git-merge" :face 'mode-line-warn-face :v-adjust -0.05))
             ((eq state 'needs-update)
              (all-the-icons-octicon "arrow-down" :face 'mode-line-warn-face :v-adjust -0.05))
             ((memq state '(removed conflict unregistered))
              (all-the-icons-octicon "alert" :face 'mode-line-error-face :v-adjust -0.05))
             (t
              (all-the-icons-octicon "git-compare" :v-adjust -0.05)))
...
;;; mu4e
(use-package mu4e
  :load-path "/home/cong/.emacs.d/elpa/mu4e/"
  :ensure mu4e-alert
  :defer t
  :commands (mu4e)
  :preface (defun *mu4e ())
